...
def main():
    # 配置：最小论文数阈值
    MIN_PAPERS = 3  # 修改这个值来改变阈值（2, 3, 4, ...）
    
    print("="*80)
    print("最小论文集合查找 - 优化版贪心算法")
    print("="*80)
    print(f"目标：覆盖所有发表过 ≥{MIN_PAPERS} 篇论文的作者")
    print("="*80)
    print()
    ...
    print(f"✓ 加载了 {len(all_papers):,} 篇论文 ({time.time()-start_time:.1f}s)\n")
    ...
    target_authors = {author for author, count in author_counts.items() if count >= MIN_PAPERS}
    
    print(f"✓ 找到 {len(target_authors):,} 位作者（至少{MIN_PAPERS}篇论文） ({time.time()-step_start:.1f}s)\n")
    ...
    with open('minimal_paper_set_report.txt', 'w', encoding='utf-8') as f:
        f.write("="*80 + "\n")
        f.write("最小论文集合 - 优化版贪心算法结果\n")
        f.write("="*80 + "\n\n")
        
        f.write(f"总论文数: {len(all_papers):,}\n")
        f.write(f"目标作者数（至少{MIN_PAPERS}篇论文）: {len(target_authors):,}\n")
